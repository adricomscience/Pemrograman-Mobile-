...
import com.example.moviefinalapp.adapters.CastAdapter;
import com.example.moviefinalapp.adapters.GenreRecyclerAdapter;
import com.example.moviefinalapp.database.AppDatabase;
import com.example.moviefinalapp.database.entities.Favorite;
import com.example.moviefinalapp.models.CastResponse;
import com.example.moviefinalapp.models.Genre;
import com.example.moviefinalapp.models.movie.Movie;
import com.example.moviefinalapp.network.Const;
import com.example.moviefinalapp.network.MovieApiClient;
import com.example.moviefinalapp.network.MovieApiInterface;
...
public class DetailActivity extends AppCompatActivity {
            ...
            case R.id.menu_like:
                ...
                if(exists){
                    ...
                    database.favoriteDao().delete(favorite).subscribe(() -> {
                        item.setIcon(ContextCompat.getDrawable(this,R.drawable.ic_add_favorite));
                    ...
                    database.favoriteDao().addFavorite(favorite).subscribe(() -> {
                        item.setIcon(ContextCompat.getDrawable(this,R.drawable.ic_added_favorite));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.movie_add_fav, menu);

        int movieId = Integer.parseInt(id);
        boolean exists = database.favoriteDao().isExists(movieId);

        if(!exists){
            menu.getItem(0).setIcon(ContextCompat.getDrawable(this,R.drawable.ic_add_favorite));
        }else{
            menu.getItem(0).setIcon(ContextCompat.getDrawable(this,R.drawable.ic_added_favorite));
